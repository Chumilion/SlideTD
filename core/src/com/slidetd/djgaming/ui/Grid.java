...
public class Grid extends Box {
  ...
  public Grid(int rows, int cols, String map) {
    ...
    TILE_SIZE = SlideTD.WIDTH / cols;
    BOARD_HEIGHT = TILE_SIZE * rows;
    BOARD_OFFSET = (SlideTD.HEIGHT - BOARD_HEIGHT) / 2;
    int picNumber = 0;
    for (int row = NUM_ROWS - 1; row >= 0; row--) {
      for (int col = 0; col <= NUM_COLS - 1; col++) {
        ...
        tileGrid[row][col]
            .setTextureRegion(SlideTD.res.getAtlas(map)
    ...
    TextureRegion blankRegion = SlideTD.res.getAtlas("kate6x6")
